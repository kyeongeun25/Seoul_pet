...
public class MainActivity extends AppCompatActivity {
    ...
    class Seoul_Delegate extends AsyncTask {
        ...
        // AsyncTask에 의해서 어떤 method가 호출되고, 모든 실행이 끝나면
    ...
    void seoul_pet(){
        ...
        try {
            ...
            if(resCode == 200){
                ...
                // reader가 데이터를 모두 읽고
                // 또 다시 readLine()을 실행하면
                // reader는 null이 되어 버린다.
                // 그러기 전에 reader 값을 sb에 보관 해 둘 것입니다.
                ...
                while(true){
                    ...
                    // 문자열 reader를 다른 문자열 변수에 임시 보관하는 방식
                    // 안드로이드에서는 문자열 = 문자열 + 다른문자열 ( =문자열 +=다른문자열) 방식은
                    // 성능상 약간의 문제가 있다고 하여 StringBuffer를 만들고
                    // sb.append(다른문자열) 형식으로 사용하라고 권장함.
                    sb.append(reader);
                    st += reader ;
                }
                ...
                // JsonData로 부터 필요한 부분만 잘라내기
                /*
                    서울시 공공 data로 부터 return받은 데이터의 구조
                    jsonData로 부터
                ...
                // AsyncTask 대리자가 실행하는 method에서는 절대 !!!!
                // view, layout 등에 직접 데이터를 표현하는 코드를 쓸 수 없다.
            ...
            //e.printStackTrace();
            Log.d("서울시","문자열을 JSON으로 변환 시키는데 오류 발생");
        }
